...
class EquipamentoController {
	...
	async localizarBeneficioPorModelo({ response, request }) {
		try {
			...
			const equipamento =
				await new EquipamentoServices().localizarBeneficioPorModelo(
					payload.modelo
				)
	...
	async localizarEquipaPorAssist24h({ response, request }) {
		try {
			...
			const equipamento =
				await new EquipamentoServices().localizarEquipaPorAssist24h(payload)
	...
	async locBaixarTodosEquipamentos({ params, response }) {
		...
		try {
			const equipamento =
				await new EquipamentoServices().locBaixarTodosEquipamentos(payload)
	...
	async endossoBaixarTodosEquipamentos({ request, response, auth }) {
		...
		try {
			const service =
				await new EquipamentoServices().endossoBaixarTodosEquipamentos(
					payload,
					auth
				)
	...
	async localizarPorCategoria({ request, response }) {
		...
		try {
			const service = await new EquipamentoServices().localizarPorCategoria(
				categoria
			)
	...
	async localizarPorSubCategoria({ params, response }) {
		...
		try {
			const service =
				await new EquipamentoServices().localizarPorSubCategoria(
					categoria_id
				)
	...
	async relatorioAdesao({ request, response }) {
		const payload = request.all()

		try {
			const service = await new EquipamentoServices().relatorioAdesao(
				payload
			)
	...
	async relatorioBaixaCancelamento({ request, response }) {
		const payload = request.all()

		try {
			const service =
				await new EquipamentoServices().relatorioBaixaCancelamento(payload)
