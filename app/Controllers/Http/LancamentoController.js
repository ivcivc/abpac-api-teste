...
class LancamentoController {
	...
	async _gerarGrupoAcordo_TMP({ request, response, auth }) {
		try {
			const model = await new LancamentoService()._gerarGrupoAcordo_TMP(
				null,
				auth
			)
	...
	async getAcordo({ params, response }) {
		try {
			const model = await new LancamentoService().getAcordo(params.id)
