...
class LocalizarController {
   ...
   async proxy({  response, request }) {
      ...
      if ( ! lodash.has(payload, 'whereStatus')) {
         ...
         payload.whereStatus= ['equipamentos.status', 'like', payload.whereStatus]
      }
      ...
      let o = { page, limit, start, count, perPage, continuar, where, whereStatus: payload.whereStatus}
   ...
   async localizarAssociado(response, payload) {
    ...
    async localizarEquipamento(response, payload) {
      ...
      try {
        const o= Database
        .from('equipamentos')
        .select('equipamentos.id','pessoas.nome', 'equipamentos.placas', 'equipamentos.status', 'pessoas.id as pessoa_id')
        ...
        .where(payload.whereStatus[0], payload.whereStatus[1], payload.whereStatus[2])
        .paginate(lodash.isUndefined(payload.page) ? 1 : payload.page, payload.limit)
