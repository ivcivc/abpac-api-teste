...
class RateioBoletoController {
	...
	async onCreate(e) {
		this.arr.push(e)
		this.socket.emit('RATEIO-BOLETO-CREATE', {
			success: true,
	...
	async onLocalizar(rateio_id) {
		...
		try {
			const model = await ModelLancamento.query()
				.select(
					'id',
					'dVencimento',
					'conta_id',
			...
			this.socket.broadcastToAll('RATEIO-BOLETO-REFRESH', {
				tipo: 'LOCALIZAR',
				success: true,
				data: model,
			})
	...
	emitBroadcast(TOPICO, DATA) {
		const topic = Ws.getChannel('rateio-boleto:*').topic('rateio-boleto:1')
	...
	async onGerarPDF(rateio_id) {
		try {
			...
			while (it) {
				...
				if (r.done) {
					it = false
					this.emitBroadcast('NOTIFICACAO', {
						tipo: 'CONCLUIDO',
						data: null,
			...
			this.emitBroadcast('NOTIFICACAO', {
				tipo: 'ERROR',
				data: null,
				message: 'Ocorreu uma falha na geração do boleto/relatorio pdf',
			})
		}
	}
				...
				try {
					...
					if (resBoleto.success) {
						...
						this.emitBroadcast('NOTIFICACAO', {
							tipo: 'ERROR',
							data: null,
							message: resBoleto.message + ' (' + item.id + ')',
						})
					}

					const topic =
						Ws.getChannel('rateio-boleto:*').topic('rateio-boleto:1')

					if (topic) {
						try {
							this.emitBroadcast('UPDATE-REFRESH', {
								tipo: 'UPDATE',
								success: true,
								data: { id: item.id, boletos },
							})
						} catch (e) {}
					}
					...
					if (topic) {
						try {
							this.emitBroadcast('UPDATE-REFRESH', {
								tipo: 'UPDATE',
								success: true,
								data: { id: item.id, isRelatorio: res },
							})
	...
	async addBoleto(registro) {
		return new Promise(async (resolve, reject) => {
			try {
				...
				const dev = Env.get('NODE_ENV')

				if (dev === 'production') {
					const service =
						await new LancamentoService().openBank_novoBoleto(
							registro,
							null
						)
					...
					resolve({
						success: Math.random() * 2 > 1 ? true : false,
						message: 'boleto - MOCK ',
					})
				}
